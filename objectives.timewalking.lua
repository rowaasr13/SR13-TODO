...
local timewalking = {
   ...
   { cataclysm       = { quest_id = { 72810, 86556 }, lfg_dungeon_id = 1146, aura_spell_id = 335150, item_turn_in_item_id = { 133377, 133378 }, item_turn_in_quest_id = { 40786, 40787 }, raid_quest_id = 57637 } },
...
local function MakeQuestGroup(args)
   local quest_ids = args.quest_ids
   local key_prefix = args.key_prefix
   local template = args.template
   local quest_start_item_ids = args.quest_start_item_ids

   local out = {}
   for quest_id_idx = 1, #quest_ids do
      local quest_data = table_merge_shallow_left({ template, { quest_id = quest_ids[quest_id_idx]} })
      if quest_start_item_ids then quest_data.quest_start_item_id = quest_start_item_ids[quest_id_idx] end
      out[#out + 1] = { [key_prefix .. quest_id_idx] = quest_data }
   end
...
for idx, key, val in array_of_pairs_iter(timewalking) do
   val.group_dungeon_quest = MakeQuestGroup({
      quest_ids = val.quest_id,
      key_prefix = 'dungeon_',
      template = table_merge_shallow_left({ timewalking_quest_template, val })
   })

   val.group_item_turn_in_quest = MakeQuestGroup({
      quest_ids = val.item_turn_in_quest_id,
      quest_start_item_ids = val.item_turn_in_item_id,
      key_prefix = 'item_turn_in_',
      template = table_merge_shallow_left({ timewalking_token_turnin_quest_template, val })
   })
...
function a_env.OutputTablesTimewalking()
   ...
   for idx, key, val in array_of_pairs_iter(timewalking) do
      ...
      output_table = a_env.CalculateObjectivesToOutputTable(val.group_item_turn_in_quest, "array_of_pairs")
      output_table.none_active = { name = output_table[1].name, info = output_table[1].info, state = output_table[1].state, period = output_table[1].period }
      a_env.OutputTableLeaveOnlyActiveQuest(output_table)
      output_tables[#output_tables + 1] = output_table
